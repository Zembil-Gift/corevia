            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {blogs.slice(0, 3).map((post) => (
                ...
                >
                  {post.coverImageUrl && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={post.coverImageUrl} alt="" loading="lazy" decoding="async" className="h-36 w-full object-cover" />
                  )}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {events.slice(0, 4).map((ev) => (
                <Link
                  ...
                  className="flex gap-4 rounded-xl border border-border bg-card p-5 transition-colors hover:border-emerald-500/40"
                >
                  {ev.coverImageUrl && (
                    // Small thumbnail on purpose: events lead with date/place, not the image.
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
